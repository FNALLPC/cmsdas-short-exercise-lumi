...
> ## References
> You can find several [papers](/cmsdas-short-exercise-lumi/reference.html#papers) with much more technical detail and several [articles](/cmsdas-short-exercise-lumi/reference.html#articles) with additional (less formal) information in the [References](/cmsdas-short-exercise-lumi/reference.html).
{: .callout}
...
During 2025, groups of more than 100 billion protons were crossed as often as 40 million times per second yielding an overall average of 60 interactions per crossing (pileup) within the CMS detector.

![Interactions per crossing (pileup) since Run 1 (pp data only)](https://cmslumi.web.cern.ch/publicplots/multiYear/pileup_allYears.png){:width="60%"}
...
Units of "barns" are also used frequently, where $$ 1 \mathrm{b} = \mathrm{10^{-24} cm^{2}} $$, a term originating from [two Purdue University physicists working on the Manhattan Project in 1942](https://www.symmetrymagazine.org/article/february-2006/hitting-the-broad-side-of-a-classified-barn).
...
> * During [2025](https://cmsoms.cern.ch/cms/run_3/pp_fills_2025), the LHC routinely delivered instantanous luminosities of $$ \approx 2 \times 10^{34} \mathrm{cm^{-2} s^{-1}} $$ $$ \left( 0.02 \mathrm{pb^{-1} s^{-1}} \right) $$ at CMS
...
![Cumulative delivered and recorded luminosity versus time since Run 1 (pp data only)](https://cmslumi.web.cern.ch/publicplots/multiYear/int_lumi_allcumulative_pp.png){:width="60%"}
...
> * During [2025-Oct-01](https://cmsoms.cern.ch/cms/summaries/daily_summary?year=2025&month=10&day=1), CMS recorded $$ \approx 1000 \mathrm{pb^{-1}} $$ during a 24-hour period
> * The total production cross section of Standard Model Higgs boson at $$ \sqrt{s} = 13.6 \mathrm{TeV} $$ is nearly $$ \approx 60 \mathrm{pb} $$ ([see table 11.2 in the 2024 PDG](https://pdg.lbl.gov/2024/reviews/rpp2024-rev-higgs-boson.pdf))
>
...
>
> $$ \mathrm{N}_{H \rightarrow \mu \mu}^{2024} = \left( \mathcal{L}_{int}^{2024} \pm \delta \right) \cdot \sigma_{\mathrm{Higgs}}^{\mathrm{total}} \cdot \mathcal{B}_{H \rightarrow \mu \mu} $$
> 
<!--
```python
lumi, uncert = 115E3, 1.6/100
L = lumi * 60 * 2E-4
n_min, n_max = L * (1-uncert), L * (1+uncert)
L * (1-uncert)**2, L * (1+uncert)**2
L-uncert*100*100, L+uncert*100*100
```
/poll "1.3 Given the luminosity uncertainty, what are the upper and lower bounds of Higgs -> dimuon produced at CMS during 2024?" "(1336, 1425)" "(1358, 1402)" "(1220, 1540)"
> > ## Solution
> > $$ \leq 115 \cdot \left( 1 + \frac{1.6}{100} \right) \cdot 10^{3} \mathrm{pb^{-1}} \cdot 60 \mathrm{pb} \cdot 2 \times 10^{-4} \approx 1358 $$\
> > $$ \geq 115 \cdot \left( 1 - \frac{1.6}{100} \right) \cdot 10^{3} \mathrm{pb^{-1}} \cdot 60 \mathrm{pb} \cdot 2 \times 10^{-4} \approx 1402 $$
> {: .solution}
-->
{: .challenge}
